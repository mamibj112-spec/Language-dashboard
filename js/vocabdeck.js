    ...
    <div class="card">
      ...
      ${content.words.map((w, i) => `
        <div style="display:flex;gap:8px;margin-bottom:10px;">
          ...
          <div style="min-width:0;color:var(--ink);font-weight:600;">${w.word} <span style="color:var(--muted);font-weight:500;font-size:12px;">${vocabdeckPron(w)}</span></div>
        </div>`).join('')}
    </div>`;
}
...
function vocabdeckWordCard(day, content, wordIdx) {
  ...
  const meaningBlock = lv >= 1 ? `
    <div style="text-align:center;font-size:28px;font-weight:700;color:var(--accent-soft);margin:16px 0 4px;word-break:keep-all;">${w.ko}</div>
    ${w.koMore ? `<div style="text-align:center;font-size:15px;color:var(--ink-soft);margin-bottom:8px;">${w.koMore}</div>` : ''}
    ${w.nuance ? `
...
function renderVocabDeckFullscreen() {
  ...
  el.innerHTML = `
    <div style="position:fixed;inset:0;background:var(--paper);z-index:9998;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:24px;text-align:center;" onclick="vocabdeckRandomNext()">
      ...
      ${w.koMore ? `<div style="font-size:18px;color:var(--ink-soft);margin-top:10px;word-break:keep-all;">${w.koMore}</div>` : ''}
